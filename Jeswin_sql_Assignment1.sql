...
SELECT SPID,PID,Amount 
FROM sales
ORDER BY Amount DESC
LIMIT 1 OFFSET 1;

-- 6. Find the salesperson who has done the least number of sales
SELECT s.SPID, p.Salesperson, COUNT(*) AS no_sales
FROM sales s
LEFT JOIN people p ON s.SPID = p.SPID
...
select s.GeoID,g.Geo,sum(Amount) as sale_amount
from sales s
left join geo g on s.GeoID=g.GeoID
group by s.GeoID,g.Geo
order by sale_amount desc;

-- 8. Show the total sales amount for each day (SaleDate wise)
select month(SaleDate) as months,day(SaleDate) as days ,year(SaleDate) as years,sum(Amount) as total_sales
from sales
group by SaleDate;
